...
const SearchBar = (props) => {
  ...
  useEffect(() => {
    //when clicking outside the search bar, the dropdown is closed
    ...
    document.addEventListener("click", (event) => {
      //when clicking outside the searchbar,
    ...
    <div className="searchbar">
      ...
      <ul id="results" className="list-group" ref={resultsRef}>
        {results.map((result, index) => {
          return (
            //each dropdown item is a link to their respective order page where the URL is /QuoteManagement/[listing ID (shortened to three letters for each ASX code)]
            <Link
              to={{
                pathname: "/QuoteManagement",
                state: { listingID: result.substring(0, 3) },
              }}
            >
              <button
                type="button"
    ...
    if (res.status === 200) {
      res.json().then((body) => {
        for (let i = 0; i < body.length; ++i) {
          searchresults.push(
            `${body[i].ListingID}` + " " + `${body[i].ListingName}`
          ); //search result format: ID Name
        }
      });
...
function CompanySearch() {
  ...
  const onInputChange = (event) => {
    ...
    setresults(
      searchresults
        .filter((result) => result.includes(event.target.value))
        .slice(0, 5) //filters through searchresults, limits result list to 5 entries
    );
  };
  ...
  return (
    <div className="companySearchTitle">
      ...
      <SearchBar results={results} onInputChange={onInputChange} />
      <br />
